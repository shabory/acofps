        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.auther.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.authers.update", [$auther->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.auther.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $auther->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('bio') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="bio" id="bio">{!! old('bio', $auther->bio) !!}</textarea>
                            @if($errors->has('bio'))
                        ...
                        <div class="form-group {{ $errors->has('user') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="user_id" id="user_id" required>
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $auther->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                        ...
                        <div class="form-group {{ $errors->has('seo_title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="seo_title" id="seo_title" value="{{ old('seo_title', $auther->seo_title) }}">
                        ...
                        <div class="form-group {{ $errors->has('seo_description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="seo_description" id="seo_description">{{ old('seo_description', $auther->seo_description) }}</textarea>
