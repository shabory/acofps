                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.courses.update", [$course->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('descrption') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="descrption" id="descrption">{!! old('descrption', $course->descrption) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('trainer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="trainer_id" id="trainer_id" required>
                                @foreach($trainers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('trainer_id') ? old('trainer_id') : $course->trainer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $course->price) }}" step="0.01" required>
                            @if($errors->has('price'))
                        ...
                        <div class="form-group {{ $errors->has('discount_price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="discount_price" id="discount_price" value="{{ old('discount_price', $course->discount_price) }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('start_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="start_date" id="start_date" value="{{ old('start_date', $course->start_date) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('end_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="end_date" id="end_date" value="{{ old('end_date', $course->end_date) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('country') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="country_id" id="country_id" required>
                                @foreach($countries as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('country_id') ? old('country_id') : $course->country->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('city') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="city_id" id="city_id">
                                @foreach($cities as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('city_id') ? old('city_id') : $course->city->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('city'))
                        ...
                        <div class="form-group {{ $errors->has('seo_title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="seo_title" id="seo_title" value="{{ old('seo_title', $course->seo_title) }}">
                        ...
                        <div class="form-group {{ $errors->has('seo_description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="seo_description" id="seo_description">{{ old('seo_description', $course->seo_description) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('speclization') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="speclization_id" id="speclization_id" required>
                                @foreach($speclizations as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('speclization_id') ? old('speclization_id') : $course->speclization->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('diploma') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="diploma_id" id="diploma_id">
                                @foreach($diplomas as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('diploma_id') ? old('diploma_id') : $course->diploma->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
